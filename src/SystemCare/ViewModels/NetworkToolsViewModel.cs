...
public partial class NetworkToolsViewModel : ObservableObject
{
    ...
    {
        await RunToolAsync(ct => _network.PingAsync(Target, AppendLine, ct));
    }

    [RelayCommand]
    private async Task TracerouteAsync()
    {
        await RunToolAsync(ct => _network.TracerouteAsync(Target, AppendLine, ct));
    }
    ...
    {
        await RunToolAsync(ct => _network.RenewIpAsync(AppendLine, ct));
    }
